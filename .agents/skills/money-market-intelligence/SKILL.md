...
- `position` on `market_indicator` places the latest value in the series' own distribution. It is a description of where this value sits relative to what this series usually does — never a signal, a forecast, or a threshold breach. Quote `percentile` with its `window_label`, because the window is part of the claim. `risk_percentile` exists only for series whose direction is declared; when it is null the series has not been classified and you must not supply a risk reading yourself. An `available: false` position carries its reason — report the reason rather than treating the absence as a neutral value, and honour any `caveat`.
