...
- ECOS·ECOS 원표·FRED·Yahoo·BoE·ECB·KRX 기반 175개 지표, 글로벌 지수 24개, 관심 종목 34개, KRX 31개 표에서 자동 발견한 52,026개 종목·계약
- 서버 렌더 시장 상황판과 시장 심리 게이지를 첫 화면으로, 브리핑·배우기·국채 곡선·KRX 전 종목 탐색·데이터 품질 감사를 포함한 14개 웹 화면과 JSON API
- 프로젝트 로컬 pi 확장·표준 MCP 도구 각 22개와 해석 품질을 통제하는 프로젝트 스킬
- 모든 계열을 자기 분포에서 채점하는 정규화 엔진, 핵심 39개 계열의 다섯 층 설명과 지금 값의 뜻 생성, 한·미 별도 국면 분류, 무엇이 바뀌면 판정이 달라지는지 보여주는 브리핑
