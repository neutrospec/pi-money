...
EXPLANATIONS = {
    # ---- 분류기 입력·watch 대상 보강 (M6 감사) ----
    "kr_semiconductor_export_volume": {
        "what": "한국 반도체 수출물량지수입니다. 단가를 제거하고 실제 출하 물량만 지수화한 것입니다.",
        "why": "메모리 사이클에서 물량과 가격은 따로 움직입니다. 감산으로 가격이 오를 때 물량은 줄 수 있고, 그 구분이 사이클 국면을 알려줍니다.",
        "how": "금액지수·가격지수와 셋을 나란히 봅니다. 물량이 먼저 돌고 가격이 따라오는 순서가 회복 국면에서 자주 관찰됩니다.",
        "watch": [
            ("kr_semiconductor_export_value", "금액 = 물량 × 단가"),
            ("kr_dram_ppi", "단가 쪽"),
            ("us_semiconductor_proxy", "글로벌 주가가 선반영했는지"),
        ],
        "caveat": "월별이며 관측일은 해당 월 1일입니다. 지수이므로 절대 수량이 아닙니다.",
    },
    "kr_export_volume_index": {
        "what": "한국 전체 수출물량지수입니다. 가격을 제거하고 실제로 나간 물량만 지수화한 것입니다.",
        "why": "금액지수만 보면 단가 상승과 물량 증가를 구분할 수 없습니다. 물량이 실물 수요의 척도입니다.",
        "how": "금액지수와 나란히 봅니다. 금액은 느는데 물량이 줄면 단가가 끌고 있다는 뜻이라 사이클 후반의 신호로 읽히는 경우가 많습니다.",
        "watch": [
            ("kr_export_value_index", "금액 대비 물량 기여"),
            ("kr_semiconductor_export_volume", "주도 품목의 물량"),
        ],
        "caveat": "월별이며 관측일은 해당 월 1일입니다. 기준연도 개편 시 과거 값이 함께 바뀝니다.",
    },
    "kr_export_value_index": {
        "what": "한국 전체 수출금액지수입니다. 금액 기준이라 물량과 단가 변화가 함께 들어 있습니다.",
        "why": "한국 경기의 가장 직접적인 척도입니다. 내수보다 수출이 먼저 움직이고, 경상수지와 기업 이익이 여기서 파생됩니다.",
        "how": "전년동월비로 읽습니다. 금액이 늘 때 물량 때문인지 단가 때문인지는 물량지수와 함께 봐야 갈립니다. 반도체가 큰 비중을 차지하므로 품목별 분해가 필요합니다.",
        "watch": [
            ("kr_export_volume_index", "물량과 단가 분리"),
            ("kr_semiconductor_export_value", "주도 품목"),
            ("kr_current_account", "대외 수지로의 귀결"),
        ],
        "caveat": "월별이며 관측일은 해당 월 1일입니다. 지수이므로 절대 금액이 아니고, 조업일수와 명절 이동에 따라 월별 변동이 큽니다.",
    },
    "kr_ppi": {
        "what": "한국 생산자물가지수입니다. 기업이 출하하는 단계의 가격이며 소비자가 내는 가격이 아닙니다.",
        "why": "수입물가와 소비자물가 사이의 중간 단계입니다. 여기서 오른 것이 시차를 두고 소비자물가로 넘어가는지가 전가의 관찰점입니다.",
        "how": "전년동월비로 읽습니다. 수입물가가 먼저 오르고 PPI가 따라 오른 뒤 CPI가 움직이는 순서가 자주 관찰됩니다. 순서가 깨지면 국내 요인을 의심합니다.",
        "watch": [
            ("kr_import_price", "상류 단계"),
            ("kr_cpi", "하류 단계"),
            ("kr_core_cpi", "기조적 전가"),
        ],
        "caveat": "월별이며 관측일은 해당 월 1일입니다. 품목 가중이 소비자물가와 달라 두 지수의 상승률을 직접 비교하면 안 됩니다.",
    },
    "kr_put_call_volume": {
        "what": "코스피200 옵션의 풋 거래량을 콜 거래량으로 나눈 비율입니다. 야간 세션은 제외하고 KRX 옵션 표에서 매일 집계합니다.",
        "why": "그날 시장이 하락에 얼마나 베팅하거나 헤지했는지를 보여줍니다. 심리 게이지의 구성요소 중 하나입니다.",
        "how": "1.0이 중립이고 높을수록 풋이 많습니다. 극단으로 치우친 뒤 되돌아오는 지점이 과거 전환과 겹친 사례가 있으나, 수준 자체가 방향을 알려주지는 않습니다.",
        "watch": [
            ("kr_put_call_open_interest", "남아 있는 포지션과의 격차"),
            ("kr_vkospi", "보험료 수준"),
            ("kr_kospi200_basis", "선물 쪽 수급"),
        ],
        "caveat": "만기일이 가까우면 단기 옵션 거래가 몰려 비율이 크게 흔들립니다. 만기 주간의 값은 심리보다 만기 효과입니다.",
    },
    "us_ig_spread": {
        "what": "미국 투자등급 회사채가 같은 만기 국채보다 얼마나 높은 금리를 주는지를 옵션조정 기준으로 계산한 값입니다.",
        "why": "우량 기업의 조달 비용입니다. 하이일드보다 늦게 반응하지만, 여기까지 번지면 스트레스가 저등급에 머물지 않는다는 뜻입니다.",
        "how": "하이일드 스프레드와 함께 봅니다. HY만 벌어지면 저등급 고유 문제이고, IG까지 따라 벌어지면 신용시장 전반입니다. 미국 국면 분류기가 이 계열을 임계값 1.0%·1.5%로 읽습니다.",
        "watch": [
            ("us_hy_spread", "저등급에 머무는지 번졌는지"),
            ("us_lqd_proxy", "실제 가격으로 본 손익"),
            ("us_nfci", "금융여건 전반"),
        ],
        "caveat": "지수 편입 종목의 등급·만기 구성이 시간에 따라 바뀌므로 아주 긴 기간의 수준 비교에는 주의가 필요합니다.",
    },
    "us_lqd_proxy": {
        "what": "미국 투자등급 회사채에 투자하는 ETF(LQD)의 가격입니다. 스프레드 수치가 아니라 거래되는 상품의 가격입니다.",
        "why": "우량 신용의 손익을 매일 보여줍니다. 하이일드 ETF와의 상대강도가 위험선호의 방향을 나타냅니다.",
        "how": "스프레드가 좁아지고 금리가 내리면 오릅니다. 금리와 신용 두 요인이 섞여 있어 방향만으로는 어느 쪽인지 알 수 없습니다.",
        "watch": [
            ("us_ig_spread", "가격을 움직이는 스프레드"),
            ("us_high_yield_proxy", "하이일드 대비 상대강도"),
            ("us_10y", "금리 요인 분리"),
        ],
        "caveat": "proxy입니다 — ETF 가격이며 공식 지수나 신용스프레드 자체가 아닙니다. 듀레이션이 길어 금리 변화만으로도 크게 움직입니다.",
    },
    "kr_current_account": {
        "what": "한국의 경상수지입니다. 상품·서비스·본원소득·이전소득의 대외 수지 합계입니다.",
        "why": "외화가 구조적으로 들어오는지 나가는지를 보여줍니다. 환율 방어력의 기초이고 외환보유액과 함께 대외 신인도를 이룹니다.",
        "how": "월별 변동이 크므로 12개월 누적으로 보는 것이 관행입니다. 흑자여도 수입 급감으로 생긴 흑자(불황형)인지 수출 호조인지 구분해야 합니다.",
        "watch": [
            ("kr_export_value_index", "수출 쪽 기여"),
            ("kr_fx_reserves", "축적 결과"),
            ("kr_usd", "환율 압력과의 관계"),
        ],
        "caveat": "월별이며 관측일은 해당 월 1일입니다. 배당 송금이 몰리는 달에는 본원소득이 크게 흔들립니다.",
    },
    "kr_core_cpi": {
        "what": "한국 소비자물가에서 농산물과 석유류를 제외한 근원 지수입니다. 수준이지 상승률이 아닙니다.",
        "why": "날씨와 유가는 통화정책으로 통제되지 않습니다. 이것들을 빼면 정책이 다룰 수 있는 기조적 물가가 남습니다.",
        "how": "전년동월비로 읽습니다. 헤드라인만 내리고 근원이 버티면 물가 둔화가 유가 덕분이라는 뜻이라 정책 전환 근거로는 약합니다.",
        "watch": [
            ("kr_cpi", "헤드라인과의 격차"),
            ("kr_base_rate", "정책 대응"),
        ],
        "caveat": "월별이며 관측일은 해당 월 1일입니다. 한국의 근원 정의는 미국과 제외 품목이 달라 수치를 직접 비교하면 안 됩니다.",
    },
    "kr_import_price": {
        "what": "한국 수입물가지수입니다. 원화 기준으로 수입품 가격을 지수화한 것이라 환율 변동이 그대로 들어 있습니다.",
        "why": "국내 물가의 상류입니다. 여기서 오른 것이 시차를 두고 생산자물가와 소비자물가로 전가됩니다.",
        "how": "전년동월비로 읽고 환율과 유가를 함께 봅니다. 원화 약세와 유가 상승이 겹치면 이 지표가 먼저 반응합니다.",
        "watch": [
            ("kr_usd", "환율 기여분"),
            ("wti", "유가 기여분"),
            ("kr_ppi", "다음 단계 전가"),
        ],
        "caveat": "월별이며 관측일은 해당 월 1일입니다. 원화 기준이라 국제 가격이 그대로여도 환율만으로 움직입니다.",
    },
    "us_core_pce": {
        "what": "미국 개인소비지출 물가지수에서 에너지와 식품을 제외한 근원 지수입니다.",
        "why": "**연준이 목표로 삼는 지표**입니다. CPI가 시장을 더 크게 움직이지만 정책 판단의 기준은 이쪽입니다.",
        "how": "전년동월비로 읽습니다. CPI와 갈릴 때가 중요한데, 두 지수는 품목 가중과 주거비 처리가 달라 방향이 다를 수 있습니다.",
        "watch": [
            ("us_core_cpi", "CPI 근원과의 격차"),
            ("us_rate", "정책 대응"),
        ],
        "caveat": "월별이며 관측일은 해당 월 1일입니다. CPI보다 늦게 발표되고 개정 폭도 큽니다.",
    },
    "us_forward_inflation_5y5y": {
        "what": "지금부터 5년 뒤 시작하는 5년 구간의 기대인플레이션입니다. 10년 기대치에서 향후 5년분을 제거해 먼 미래만 남긴 값입니다.",
        "why": "단기 물가 충격에 덜 흔들려서 중앙은행이 기대를 붙잡고 있는지 보는 데 쓰입니다. 여기가 움직이면 시장이 정책의 장기 신뢰를 다시 매긴 것입니다.",
        "how": "10년 기대치는 오르는데 이것이 안정적이면 단기 충격으로 읽히고, 이것까지 오르면 기대가 풀린 신호로 해석됩니다.",
        "watch": [
            ("us_breakeven_10y", "단기 포함 기대치와의 격차"),
            ("us_real_10y", "실질금리 쪽 반응"),
        ],
        "caveat": "두 기대치의 차이에서 유도한 값이라 각각의 유동성 프리미엄 오차가 증폭됩니다.",
    },
    "kr_corp_bond_bbb": {
        "what": "신용등급 BBB− 회사채 3년물의 유통수익률입니다. 투자등급의 최하단이며 그 아래는 투기등급입니다.",
        "why": "국고채와의 차이가 저등급 기업의 실제 조달 여건입니다. 우량 등급 스프레드가 좁아도 여기가 벌어져 있으면 신용시장이 등급별로 갈린 것입니다.",
        "how": "AA− 스프레드와 함께 봅니다. 심리 게이지의 신용 구성요소가 이 계열의 국고 대비 스프레드를 자체 분포 백분위로 채점합니다.",
        "watch": [
            ("kr_corp_bond_3y", "우량 등급과의 격차가 등급별 분화"),
            ("kr_treasury_3y", "차이가 곧 스프레드"),
        ],
        "caveat": "국내 BBB− 시장은 발행과 거래가 얇아 수준이 구조적으로 높습니다. 미국 하이일드 스프레드와 수치를 직접 비교하면 안 됩니다.",
    },
    "kr_treasury_30y": {
        "what": "잔존 30년 국고채 지표종목의 유통수익률입니다. 국내에서 가장 만기가 긴 무위험금리입니다.",
        "why": "보험사·연기금의 부채 듀레이션 수요가 집중되는 구간이라, 경기 전망보다 수급이 가격을 정하는 일이 잦습니다.",
        "how": "10년·20년과 함께 곡선으로 봅니다. 30년이 20년보다 낮으면(역전) 경기 비관보다는 초장기 수요 쏠림일 가능성이 큽니다.",
        "watch": [
            ("kr_treasury_10y", "장기 구간 기울기"),
            ("kr_treasury_20y", "중간 만기와의 관계"),
            ("kr_bond_duration", "시장 전체의 금리 민감도"),
        ],
        "caveat": "거래가 얇아 하루 변동이 크고, 수급 요인이 경기 신호로 오독되기 쉽습니다.",
    },
    "kr_put_call_open_interest": {
        "what": "코스피200 옵션의 풋 미결제약정을 콜 미결제약정으로 나눈 비율입니다. 야간 세션은 제외합니다.",
        "why": "거래량 비율이 하루의 심리라면 미결제 비율은 **남아 있는 포지션**의 구성입니다. 헤지가 쌓여 있는지 보여줍니다.",
        "how": "1.0이 중립이고 높을수록 풋이 많습니다. 거래량 비율과 갈릴 때가 정보인데, 거래는 콜 쪽인데 미결제는 풋 쪽이면 기존 헤지가 유지된 채 단기 방향 베팅이 붙은 것입니다.",
        "watch": [
            ("kr_put_call_volume", "당일 거래 쪽 심리와의 격차"),
            ("kr_vkospi", "보험료 수준"),
            ("kr_kospi200_futures_oi", "선물 쪽 포지션 규모"),
        ],
        "caveat": "만기일 전후로 미결제가 기계적으로 정리되면서 비율이 크게 흔들립니다. 롤오버 구간의 변화는 심리 변화가 아닙니다.",
    },
    "kr_vkospi": {
        "what": "코스피200 옵션 가격에서 역산한 향후 30일 기대 변동성입니다. 실제 변동성이 아니라 시장이 지불하는 보험료이며, KRX 파생상품지수 표의 320개 행 중 하나를 계열로 끌어올린 것입니다.",
        "why": "한국 시장의 공포를 실시간으로 보여줍니다. VIX가 미국에 하는 역할을 국내에서 합니다. 국면 분류기와 심리 게이지가 둘 다 이 계열을 읽습니다.",
        "how": "수준이 낮다고 안전한 것이 아니라 시장이 안전하다고 믿는다는 뜻입니다. 미국 VIX와 벌어질 때가 중요합니다 — 한쪽만 튀면 그 시장 고유의 문제입니다.",
        "watch": [
            ("us_vix", "미국과 갈리는지가 한국 고유 문제인지 알려줍니다"),
            ("kr_put_call_volume", "옵션 시장의 헤지 수요"),
        ],
        "caveat": "평균회귀하는 계열이라 최근 1년이 아니라 전체 이력을 기준으로 위치를 봅니다. 위기를 포함한 1년 창을 쓰면 위기가 스스로를 정상으로 채점합니다.",
    },
    # ---- KRX 파생 (M6.3) ----
    "kr_breakeven_10y": {
