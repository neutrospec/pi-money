...
**답할 수 있는 범위는 좁고, 그 사실을 화면이 보고합니다.** 원장 24,017행 중 98%가 사후 백필입니다. 백필된 행은 "지금 그 날짜에 대해 갖고 있는 값"을 증명하지 그 날짜에 갖고 있었음을 증명하지 않고, 대신 백필 **이후**의 모든 날짜를 그 깊이로 재생 가능하게 만듭니다. 그래서 "언제 충분해지나"는 추정이 아니라 원장에 이미 적힌 사실입니다 — N번째 관측이 도착한 시각이 곧 깊이가 채워진 시각입니다. `/data`가 계열마다 그 날짜를 보고합니다. 2026-08-29 현재 브리핑 입력 7개 중 2개만 재생 가능합니다.
