...
| 단계 | 내용 | 선행 |
|---|---|---|
| **M6.1** | 정규화 엔진 추출. 모든 타일이 백분위를 가짐 | 없음 |
| **M6.2** | 카탈로그 설명 필드 + 핵심 38개 작성 + `?` 진입점 + `/learn` | M6.1 (깊이 3이 백분위 필요) |
| **M6.3** | 파생 계열 승격 — 기대인플레이션 우선, 이어서 포지셔닝·괴리율·듀레이션 | 없음 (병렬 가능) |
| **M6.4** | `/brief` — 불일치·watch 조건·근거 링크 | M6.1~M6.3 |
