...
**보류한 둘.** ETF 순자산 총액은 변화의 대부분이 자금 유출입이 아니라 평가손익이라 이 표만으로는 흐름 proxy로 쓸 수 없습니다. 채권지수 평균수익률은 이미 만기별로 보유한 곡선의 가중평균에 가까워 새 정보가 적어, 같은 표에서 듀레이션만 취했습니다.
...
| **M6.2** | 카탈로그 설명 필드 + 핵심 38개 작성 + `?` 진입점 + `/learn` | M6.1 (깊이 3이 백분위 필요) | **완료 (2026-08-29)** |
| **M6.3** | 파생 계열 승격 — 기대인플레이션 우선, 이어서 포지셔닝·괴리율·듀레이션 | 없음 (병렬 가능) | **완료 (2026-08-29)** |
| **M6.4** | `/brief` — 불일치·watch 조건·근거 링크 | M6.1~M6.3 | **완료 (2026-08-29)** |
